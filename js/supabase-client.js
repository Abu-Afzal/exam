...
// ============================================
// HELPER: DATABASE (Pengganti LocalStorage)
// ============================================
const DB = {
  async getUjian() {
    const { data, error } = await supabase.from('ujian').select('*');
    if (error) throw error;
    return data;
  },
  ...
  async saveUjian(ujian) {
    const { data, error } = await supabase.from('ujian').insert(ujian).select().single();
    if (error) throw error;
    return data;
  },

  async getSoal(mapel) {
    const { data, error } = await supabase.from('bank_soal').select('*').eq('mapel', mapel);
    if (error) throw error;
    return data;
  },

  async saveSoal(soal) {
    const { data, error } = await supabase.from('bank_soal').insert(soal).select().single();
    if (error) throw error;
    return data;
  },
  ...
  async saveHasil(hasil) {
    const { data, error } = await supabase.from('hasil').insert(hasil).select().single();
    if (error) throw error;
    return data;
  },
